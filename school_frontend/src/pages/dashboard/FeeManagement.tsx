...
import { fetchFeeDueByAdmissionNo, fetchAllFeeDues } from "@/store/slices/feeDuesSlice";
...
const FeeManagement = () => {
  ...
  const feeDues = useSelector((state: RootState) => state.feeDues);
  const previousDueAmount = feeDues?.dueAmount ?? 0;
  const dueLoading = feeDues?.loading ?? false;
  const allFeeDues = feeDues?.allDues ?? {}; // Map of admission_no -> due_amount
  ...
  const isAdmin = useMemo(() => role === "admin" || role === "finance", [role]);
  ...
  useEffect(() => {
    ...
    dispatch(fetchFeeDueByAdmissionNo(String(student.admission_no)));
  }, [selectedStudent, students, dispatch]);
  ...
  useEffect(() => {
    if (!isAdmin) return;
    dispatch(fetchAllFeeDues()); // Fetch all fee dues for status calculation
  }, [dispatch, isAdmin]);
  ...
  if (selectedStudent && localFeeStructure.length > 0 && allPayments.length > 0) {
    ...
    if (student) {
      ...
      if (status) {
          ...
          // Get previous due amount from fee_dues table for this student
          const previousDueFromDB = Number(allFeeDues[String(student.admission_no)] || 0);
...
useEffect(() => {
    ...
    if (student) {
      ...
      if (feeStruct) {
        ...
        for (const feeType of oneTimeFeeTypes) {
          ...
          else if (feeType === "Admission Fee") totalAmount += Number(feeStruct.admissionFee) || 0;
        ...
        totalAmount = totalAmount + (previousDueAmount || 0); // Add previous dues
  ...
  // Export Payment History to Excel with Enhanced Details
  const exportPaymentHistoryToExcel = useCallback(() => {
    try {
      ...
      const data = allPayments.map(payment => ({
        ...
        'Subtotal (₹)': (payment.monthly_fees || 0) + (payment.bus_fee || 0) + (payment.admission_fees || 0) + 
                        (payment.exam_fees || 0) + (payment.dress_fee || 0) + (payment.book_fee || 0) + 
                        (payment.fine || 0) + (payment.other_fee || 0),
        'Discount (₹)': payment.discount || 0,
        'Total Amount Paid (₹)': payment.totalAmount,
      ...
      // Create worksheet with auto-sizing
      ...
      const filename = `Payment_History_Detailed_${timestamp}.xlsx`;
      
      XLSX.writeFile(wb, filename);
      toast.success(`Payment history exported! File: ${filename}`);
    } catch (error) {
      console.error("Export Error:", error);
      toast.error("Failed to export Excel sheet.");
    }
  }, [allPayments, resolveAdmissionNo]);

  // Export Dues List
  const exportDuesListToExcel = useCallback(() => {
    try {
  ...
  }, [filteredStudents, studentsFeeStatus]);

  // --- Generate Due List PDF ---
const generateDueSlipPDF = async (targetStudents: StudentFeeStatus[], isBulk = false) => {
    // Compact page size - only reduce height, keep width same
    const pdf = new jsPDF({
      orientation: 'portrait',
      unit: 'mm',
      format: [160, 210] // Width: 210mm (A4 full width), Height: 180mm (reduced from 297mm)
    });
    const pageWidth = 160;
    const margin = 15;
  ...
  const handleSaveFeeStructure = useCallback(async () => {
  ...
  const handleCollectFee = async () => {
    ...
    const admissionFeeVal = selectedFeeTypes.includes("Admission Fee") ? (Number(feeStruct.admissionFee) || 0) : 0;
    ...
    for (let i = 0; i < monthsToProcess.length; i++) {
      ...
      // record.totalAmount = Number(paymentAmount);

      if(i==0){
        record.paid_amount = Number(paidAmount);
      }else{
        record.paid_amount = 0;
      }
      
      record.due_amount = Number(dueAmount);
  ...
  const generateCombinedFeeReceiptPDF = async (payments: any[], student: any, pendingInfo?: StudentFeeStatus) => {
    ...
    // Due Amount Section
    if(previousDueAmount<0){ //advance from previous
      pdf.setTextColor(0, 100, 0);
      pdf.text("Previous Advance Amount:", margin + 5, yPos);
      pdf.text(`- Rs. ${Math.abs(previousDueAmount).toLocaleString()}`, pageWidth - margin - 5, yPos, { align: "right" });
      yPos += 7;
    }else if(previousDueAmount>0){ //due from previous
      pdf.setTextColor(200, 0, 0);
      pdf.text("Previous Month Due:", margin + 5, yPos);
      pdf.text(`+ Rs. ${previousDueAmount.toLocaleString()}`, pageWidth - margin - 5, yPos, { align: "right" });
      yPos += 7;
    }
    ...
    if( dueAmount > 0 ){
      pdf.setTextColor(200, 0, 0);
      pdf.text("Due Amount:", margin + 5, yPos);
      pdf.text(`- Rs. ${dueAmount.toLocaleString()}`, pageWidth - margin - 5, yPos, { align: "right" });
      yPos += 7;
    }else if(dueAmount<0){
      pdf.setTextColor(0, 100, 0);
      pdf.text("Advance Amount:", margin + 5, yPos);
      pdf.text(`+ Rs. ${Math.abs(dueAmount).toLocaleString()}`, pageWidth - margin - 5, yPos, { align: "right" });
      yPos += 7;
    }
    ...
    pdf.text(`Rs. ${paidAmount.toLocaleString()}`, pageWidth - margin - 5, yPos, { align: "right" });
    ...
    if (pendingInfo) {
      ...
      const totalPreviousDues = Number(pendingInfo.pendingAmount || 0) + 
        ...
        Number(previousDueAmount || 0);
      
      // Total Amount Paid in Current Transaction (before discount)
      const totalAmountPaidNow = grandTotalPaid;
      
      // Remaining Balance = Total Previous Dues - Total Amount Paid - Total Discount Applied
      const previousAdvance = Number(pendingInfo.advanceAmount || 0);
      
      // Formula: REMAINING BALANCE = TOTAL PREVIOUS DUES - TOTAL AMOUNT PAID - Total Discount Applied
      let remainingBalance = totalPreviousDues - paidAmount - totalDiscount;
      ...
      pdf.text(`TOTAL AMOUNT PAID: Rs. ${paidAmount.toLocaleString()}`, margin, yPos);
      yPos += 7;
  ...
  const collection = useMemo(() => {
    if (allPayments.length === 0) return { total: 0, thisMonth: 0, thisYear: 0, today: 0 };
    return getTotalCollection(allPayments);
  }, [allPayments.length]);
  ...
  // Filter students for collection search safely - LIMIT to 50 for performance
  const filteredCollectStudents = useMemo(() => {
    if (!collectSearchTerm || collectSearchTerm.length < 2) return [];
    ...
    return filtered.slice(0, 50); // Show max 50 results
  }, [students, collectSearchTerm]);

  // Filter students for history search - LIMIT to 50 for performance
  const filteredHistoryStudents = useMemo(() => {
    if (!admissionNoSearch || admissionNoSearch.length < 2) return [];
    ...
    return filtered.slice(0, 50); // Show max 50 results
  }, [students, admissionNoSearch]);

  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* Loading Overlay */}
      {(loading || structLoading || isCalculating) && (
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                 <div>
                   ...
                   <div className="relative">
                      ...
                      />
                      {showSuggestions && collectSearchTerm && collectSearchTerm.length >= 2 && !selectedStudent && (
                        <div className="absolute z-50 w-full bg-white border rounded-md shadow-lg max-h-60 overflow-auto mt-1">
                           {filteredCollectStudents.length > 0 && (
                             <div className="p-2 bg-gray-50 border-b text-xs text-gray-600 font-medium">
                               Showing {filteredCollectStudents.length} result(s) {filteredCollectStudents.length === 50 ? '(max 50)' : ''}
                             </div>
                           )}
                      ...
                      {showSuggestions && collectSearchTerm && collectSearchTerm.length < 2 && !selectedStudent && (
                        <div className="absolute z-50 w-full bg-white border rounded-md shadow-lg p-3 mt-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                 <Label>Final Payable Amount: {selectedFeeTypes.some(f => f !== "Monthly Fee") && paymentMonths.length === 0 ? "(Auto-calculated)" : ""}</Label>
                 <Input 
                   value={paymentAmount} 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input type="number" value={paidAmount} onChange={e => setPaidAmount(Number(e.target.value))} placeholder="e.g. 500" />
                  <p className="text-xs text-gray-500 mt-1">Amount Which is paying By Student</p>
                </div>
                  ...
                  <div className="space-y-2">
                    {(() => {
                      ...
                      if (selectedFeeTypes.includes("Admission Fee")) {
                        const rate = Number(feeStruct.admissionFee) || 0;
                      ...
                      const finalTotal = subtotal - (Number(discountAmount) || 0) + previousDueAmount;                     
                        ...
                        <>
                          ...
                          {previousDueAmount != 0 && (
                            <div className="flex justify-between text-sm text-red-600">
                              ...
                              {previousDueAmount < 0 ? (
                                 <span className="text-green-600">Previous Advance Adjustment:</span>
                              ) : (
                                 <span>Previous Due Amount:</span>
                              )}                          
                              
                              {previousDueAmount < 0 ? (
                                <span className="text-green-600">-₹{Math.abs(previousDueAmount).toLocaleString()}</span>
                              ) : (
                                <span>+₹{previousDueAmount.toLocaleString()}</span>
                              )}
                            </div>
                          )}
                          ...
                          <div className="flex justify-between text-base font-bold text-yellow-700 bg-yellow-50 p-2 rounded mt-2">
                              ...
                              <TableRow key={s.admissionNo}>
                                 ...
                                 <TableCell>
                                     <Badge variant={s.totalPreviousDues > 0 ? "destructive" : "default"} className={s.totalPreviousDues <= 0 ? "bg-green-600" : ""}>
                                         {s.totalPreviousDues > 0 ? "Pending" : s.totalPreviousDues < 0 ? "Advance" : "Paid"}
                                     </Badge>
                                 </TableCell>
                                 <TableCell>
                         ...
                         .map(s => {
                           ...
                           return (
                              <TableRow key={s.admissionNo} className="bg-red-50/30">
                                 ...
                                 <TableCell className="text-red-700 font-bold text-lg">
              ...
              <CardContent className="space-y-4">
                 <div className="flex gap-2 items-start">
                    <div className="relative flex-1">
                        ...
                        />
                        {showHistorySuggestions && admissionNoSearch && admissionNoSearch.length >= 2 && (
                            <div className="absolute z-50 w-full bg-white border rounded-md shadow-lg max-h-60 overflow-auto mt-1">
                               {filteredHistoryStudents.length > 0 && (
                                 <div className="p-2 bg-gray-50 border-b text-xs text-gray-600 font-medium">
                                   Showing {filteredHistoryStudents.length} result(s) {filteredHistoryStudents.length === 50 ? '(max 50)' : ''}
                                 </div>
                               )}
